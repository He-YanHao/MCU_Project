...
void key_gpio_config(void)
{
	...
	nvic_irq_enable(BSP_KEY_EXTI_IRQN,2U,2U);  // 抢占优先级2，子优先级2
	/* 连接中断线到GPIO */
  ...
  if(exti_interrupt_flag_get(BSP_KEY_EXTI_LINE) == SET)   // 中断标志位为1，按键按下	
	{
		...
		{
			 /* 按键按下操作的功能 */ 
			sprintf(buff, "key press %d ",key_count++);
			tli_show_string(800/2-24, 480-16, WHITE, BLACK,1,(uint8_t*)buff,0);	
			printf("key press!\r\n");                              // 串口打印key press!
		}else{                                                   // 按键释放 
			/* 按键松开操作的功能 */ 
			printf("key release!\r\n");                            // 串口打印key release!
...
void key_scan(void)
{
	...
	if(gpio_input_bit_get(BSP_KEY_PORT,BSP_KEY_PIN) == SET)         // 按键按下
	{
		...
		{
			tli_show_string(0, 0, WHITE, BLACK,1,(uint8_t*)"KEY OK",0);	
			printf("key press!\r\n");								// 串口打印key press!
