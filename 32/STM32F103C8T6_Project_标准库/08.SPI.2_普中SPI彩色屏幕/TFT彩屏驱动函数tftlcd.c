...
_tftlcd_data tftlcd_data;//申请结构体
...
{
	TFT_WR=1;//要写入数据，先激活WR
	TFT_CS=0;//片选，低电平选中LCD
	TFT_RD=1;//不从LCD读取数据
 	TFT_RS=0;//命令数据选择，高，命令
 	TFT_DATAPORTL=cmd;//要写入命令
	TFT_WR=0;//写入命令完毕，取消激活WR
 	TFT_WR=1;//写入命令激活WR
 	TFT_CS=1;//片选，高电平不选中LCD
}
...
void LCD_WriteData(u16 dat)
{
	...
	TFT_CS=0;//片选，低电平选中LCD
	TFT_RD=1;//不从LCD读取数据
 	TFT_RS=1;//命令数据选择，低，数据
 	TFT_DATAPORTL=dat;//要写入数据
	TFT_WR=0;//写入数据完毕，取消激活WR
 	TFT_WR=1;//写入数据激活WR
 	TFT_CS=1;//片选，高电平不选中LCD
}

void LCD_WriteData_Color(u16 color)//LCD写颜色
{
	TFT_WR=1;//要写入数据，先激活WR
	TFT_CS=0;//片选，低电平选中LCD
	TFT_RD=1;//不从LCD读取数据
