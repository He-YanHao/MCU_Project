...
void Task1(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        {
            printf("事件组标志位1被置位\r\n");
            xEventGroupSetBits(EvGrHandle, 0x1);
        }
        else if(key == KEY1)
        {
            printf("事件组标志位2被置位\r\n");
            xEventGroupSetBits(EvGrHandle, 0x2);
        }
        vTaskDelay(50);
    }
}
...
void Task2(void *pvParameters)
{
    ...
    while(1)
    {
        xEventGroupWaitBits(EvGrHandle, 0x3, pdFALSE, pdTRUE, portMAX_DELAY);
        num = xEventGroupGetBits(EvGrHandle);
        printf("事件组的当前值：%u\r\n",num);
        xEventGroupClearBits(EvGrHandle, 0x3);
        vTaskDelay(50);
    }
}
