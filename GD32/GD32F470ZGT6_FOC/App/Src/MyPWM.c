...
void MyPWM_Init(void)
{
    ...
    timer_initpara.prescaler = 5 - 1;//预分频值（0~65535）
    ...
    timer_initpara.repetitioncounter = 0;//重复计数器值（0~255）
    ...
    //配置通道TIMER1_CH1
    timer_channel_output_pulse_value_config(TIMER1,TIMER_CH_1,2048-1);              //输出比较寄存器
    timer_channel_output_mode_config(TIMER1,TIMER_CH_1,TIMER_OC_MODE_PWM0);         //配置外设TIMERx通道输出比较模式
    timer_channel_output_shadow_config(TIMER1,TIMER_CH_1,TIMER_OC_SHADOW_DISABLE);  //配置TIMERx通道输出比较影子寄存器功能
    //配置通道TIMER1_CH2
    timer_channel_output_pulse_value_config(TIMER1,TIMER_CH_2,3096-1);              //输出比较寄存器
    timer_channel_output_mode_config(TIMER1,TIMER_CH_2,TIMER_OC_MODE_PWM0);         //配置外设TIMERx通道输出比较模式
    timer_channel_output_shadow_config(TIMER1,TIMER_CH_2,TIMER_OC_SHADOW_DISABLE);  //配置TIMERx通道输出比较影子寄存器功能
    //配置通道TIMER1_CH3
    timer_channel_output_pulse_value_config(TIMER1,TIMER_CH_3,4095-1);              //输出比较寄存器
    timer_channel_output_mode_config(TIMER1,TIMER_CH_3,TIMER_OC_MODE_PWM0);         //配置外设TIMERx通道输出比较模式
    timer_channel_output_shadow_config(TIMER1,TIMER_CH_3,TIMER_OC_SHADOW_DISABLE);  //配置TIMERx通道输出比较影子寄存器功能
