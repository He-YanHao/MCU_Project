...
*/
/*
    \从指定 SD卡地址读取一个数据块到缓冲区
    \输出参数 指向存储读取数据的缓冲区的指针
    \输入参数 要读取的数据在 SD 卡上的地址
    \输入参数 数据块的大小（单位通常是字节）
    \返回值 返回 SD 卡操作状态，类型为 sd_error_enum（枚举形式的错误/状态码）
*/
sd_error_enum sd_block_read(uint32_t *preadbuffer, uint32_t readaddr, uint16_t blocksize)
{
    ...
    // 第一个参数：0 传输数据的长度，或者 块计数 / 数据寄存器选择
    // 第二个参数：0 通常表示 传输方向 或 是否启用 DMA 0 常表示 从卡读取数据（卡 → MCU） 或默认配置
    // 第三个参数：SDIO_DATABLOCKSIZE_1BYTE 配置 每次传输的数据块大小 1BYTE → 每个数据块为 1 字节
    sdio_data_config(0, 0, SDIO_DATABLOCKSIZE_1BYTE);
    // 配置 SDIO 本次传输为“块模式”，数据方向是写入 SD 卡
    sdio_data_transfer_config(SDIO_TRANSMODE_BLOCK, SDIO_TRANSDIRECTION_TOCARD);
    // 禁用 DSM（Data Stream Mode / Data Stream Management）
    sdio_dsm_disable();
    // 禁用 DMA 数据传输
    sdio_dma_disable();
    ...
    /* 处理 SDHC/SDXC 高容量卡的地址和块大小问题 */
    // 普通 SD 卡按字节寻址，高容量卡按 512B 块寻址，这段代码做了兼容转换。
    if(SDIO_HIGH_CAPACITY_SD_CARD == cardtype) {
        blocksize = 512;
        readaddr /= 512;
    }

    // 为 SD 卡设置数据块长度 并进行 合法性检查和错误处理
...
*/
/*!
    \brief 从指定地址读取多个数据块到缓冲区
    \param[out] preadbuffer 指向存储读取的多个数据块的缓冲区的指针
    \param[in] readaddr 要读取的数据起始地址
    \param[in] blocksize 每个数据块的大小
    \param[in] blocksnumber 要读取的数据块数量
    \retval sd_error_enum 返回 SD 卡操作状态（枚举类型）
*/
...
/*
    功能描述：将一个数据块写入到卡片的指定地址。
    输入参数：
        pwritebuffer：指向存储要传输的数据块的指针。
        writeaddr：写入数据的地址。
        blocksize：数据块的大小。
    输出参数：无。
    返回值：
        sd_error_enum（SD卡操作错误状态枚举类型
*/
...
/*
    函数功能：
        ...
        该函数用于获取 SD 卡的状态，具体是获取 R1 格式的响应。
        SD 卡的状态信息通常包含很多关于卡当前工作状态的数据，如是否处于忙碌状态、是否有错误等。
    参数说明：
        pcardstatus：指向一个 uint32_t 类型的指针，函数将通过该指针返回 SD 卡的状态信息。
    返回值：
        sd_error_enum：返回一个枚举类型的值，表示函数执行的结果。
*/
sd_error_enum sd_cardstatus_get(uint32_t *pcardstatus)
{
    ...
    /* 发送 CMD13（SEND_STATUS）命令，让指定的卡返回它的状态寄存器 */
    sdio_command_response_config(SD_CMD_SEND_STATUS, (uint32_t)sd_rca << SD_RCA_SHIFT, SDIO_RESPONSETYPE_SHORT);
    sdio_wait_type_set(SDIO_WAITTYPE_NO);
    sdio_csm_enable();
    /* check if some error occurs */
...
/*
    函数说明：该函数的目的是通过获取 SD 卡的 CID（卡识别号）和 CSD（卡特性描述符）寄存器的信息，
        来填充一个 sd_card_info_struct 类型的结构体，从而获取 SD 卡的详细信息。
    
    参数说明：
        pcardinfo：一个指向 sd_card_info_struct 类型的指针，用来存储从 SD 卡获取的详细信息。
    返回值：
        sd_error_enum：返回一个枚举类型的值，表示函数执行的结果，可能的值包括：
        SD_STATUS_OK：操作成功
        SD_CMD_RESP_TIMEOUT：命令响应超时
        SD_ERROR：其他错误（具体错误类型需要根据实现来定义）
*/
...
/*!
    ...
    \param[in]  buswidth: 总线宽度
      \arg        SD_BUS_WIDTH_1BIT: 1位总线宽度
      \arg        SD_BUS_WIDTH_4BIT: 4位总线宽度
    \param[out] none
    \retval     sd_error_enum
*/
static sd_error_enum sd_bus_width_config(uint32_t buswidth)
...
/*!
    \brief      获取对应卡的 SCR（卡状态寄存器）
    \param[in]  rca: 卡的 RCA（相对卡地址）
...
static void gpio_config(void)
{
    /* 配置 SDIO_DAT0 (PC8), SDIO_DAT1 (PC9), SDIO_DAT2 (PC10), SDIO_DAT3 (PC11), SDIO_CLK (PC12) 和 SDIO_CMD (PD2) */
...
sd_error_enum BSP_SD_card_init(void)
{
    ...
    status = sd_init();
    if(SD_OK == status) {
        // 获得配置 但是不确定这个配置怎么用 以及这个时候获得的配置是否完整
...
//        status = sd_bus_mode_config(SDIO_BUSMODE_1BIT);
    ...
    if(SD_OK == status) {
        // 配置 DMA模式 或 轮询模式
//        status = sd_transfer_mode_config(SD_DMA_MODE); // DMA 模式
        status = sd_transfer_mode_config(SD_POLLING_MODE); // 轮询模式
    }
    return status;
}

/*!
    \brief      获取SD卡信息并通过USART打印出来
    \param[in]  none
    \param[out] none
    \retval     none
*/
sd_error_enum card_info_get(void)
{
    ...
    if(2 == sd_security) {
        ...
        printf("\r\n## card sd_security is : %d##", sd_security);
    }

    block_count = (sd_cardinfo.card_csd.c_size + 1) * 1024;
    block_size = 512;
    if (sd_card_capacity_get() < 128) {
        printf("\r\n## Device size is %dKB ##", sd_card_capacity_get());
    } else if (sd_card_capacity_get() < 128 * 1024) {
        printf("\r\n## Device size is %dMB ##", sd_card_capacity_get()/1024);
    } else if (sd_card_capacity_get() < 128 * 1024 * 1024) {
        printf("\r\n## Device size is %dGB ##", sd_card_capacity_get()/1024/1024);
    }
...
//#define BUF_SIZE 256
//#define ADDRESS1 512*1
//#define ADDRESS2 512*0
//#define ADDRESS3 512*3

//// 测试SD卡
//sd_error_enum sd_test(void)
//{
//    static uint32_t buf_write[BUF_SIZE]; // 该数组用于存储要写入到 SD 卡的数据
//    static uint32_t buf_read [BUF_SIZE]; // 数组用于存储从 SD 卡读取的数据
//    // 准备SD卡错误变量
//    sd_error_enum sd_error;

//    /* 初始化写入的数据 */
//    for(uint16_t i = 0; i < sizeof(buf_write); i++) {
//        buf_write[i] = i;
//    }
//    memset(buf_read, 0, sizeof(buf_read));

//    // 把buf_write写到ADDRESS1，每个数据块512字节
//    sd_error = sd_block_write(buf_write, ADDRESS1, 512);
//    if(SD_OK != sd_error) {
//        printf("\r\n Block write fail! \r\n");
//    } else {
//        printf("\r\n Block write success! \r\n");
...
//    // 从ADDRESS1读取buf_read，每个数据块512字节，并输出。
//    sd_error = sd_block_read(buf_read, ADDRESS1, 512);
//    if(SD_OK != sd_error) {
...
//    {
//        printf("write & read is OK! \r\n");
//    } else {
//        printf("write & read is NO! \r\n");
//    }

//    // 测试多个数据块的写操作
//    sd_error = sd_multiblocks_write(buf_write, ADDRESS2, 512, BUF_SIZE/128);
//    if(SD_OK != sd_error) {
...
//    // 测试多个数据块的读操作
//    memset(buf_read, 0, sizeof(buf_read));
//    sd_error = sd_multiblocks_read(buf_read, ADDRESS2, 512, BUF_SIZE/128);
//    if(SD_OK != sd_error) {
//        printf("Multiple block read fail! \r\n");
//    } else {
//        printf("Multiple block read success! \r\n");
//    }
//    // 从ADDRESS2读取buf_read，每个数据块512字节，并输出。
//    sd_error = sd_block_read(buf_read, ADDRESS1, 512);
//    if(SD_OK != sd_error) {
//        printf("Block read fail! \r\n");
//    } else {
...
//            printf("%d ",buf_read[i]);
//            if((i+1)%20 == 0 || i == BUF_SIZE-1) {
//                printf("\r\n");
//            }
//        }
//    }
//    // 比较读取与写入是否相同
//    if(memcmp(buf_read, buf_write, BUF_SIZE*4) == 0)
//    {
//        printf("write & read is OK! \r\n");
//    } else {
//        printf("write & read is NO! \r\n");
