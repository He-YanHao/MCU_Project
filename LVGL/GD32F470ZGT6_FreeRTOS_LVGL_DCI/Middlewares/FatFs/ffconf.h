...
// 使能文件系统挂载函数 f_mkfs()，将编译宏_USE_MKFS 的值定义为 1
...
// 修改支持硬件驱动的数量 3，本次移植支持三种存储介质（SD 卡、SPI_Flash、片上 Flash）
#define FF_VOLUMES		1
...
/* 这一组选项配置了可支持的扇区大小范围（512、1024、2048 或 4096 字节）。
   对于大多数系统、通用存储卡和硬盘，通常将两者都设置为 512。
   但是对于板载闪存或某些类型的光盘，可能需要更大的值。
   当 FF_MAX_SS 大于 FF_MIN_SS 时，FatFs 被配置为可变扇区大小模式，
   此时 disk_ioctl() 需要实现 GET_SECTOR_SIZE 命令以返回当前扇区大小。 */
